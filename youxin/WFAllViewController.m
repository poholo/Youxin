...
#import "WFAllViewController.h"
#import "UIUtils.h"

#import "AlertUtils.h"
#import "DateUtils.h"
#import "ViewUtils.h"
@interface WFAllViewController ()


@end

@implementation WFAllViewController
...
-(void)downloadComplete:(HttpDownload *)hd{
    ...
    if(dict==NULL){
        ...
        self.allSum = [[dict objectForKey:@"ljTotal"] intValue];
        self.cancelSum = [[dict objectForKey:@"qxTotal"] intValue];
        self.addSum = [[dict objectForKey:@"xzTotal"] intValue];
    ...
    if(_fromYear==_nowYear){
            ...
            switch (splitByMonth) {
                case 1:
                {
                    ...
                    if(fromArr==NULL){
                        ...
                        int ySumOfWeek;
                        for(int i=0;i<sumOfWeek;i++){
                            ySumOfWeek = [self getySunOfWeekFromMonth:xMonth fromDay:x fromYear:xFromYear arr:fromArr sumkey:@"dayTotal" datekey:@"dayTime"]<0?0:[self getySunOfWeekFromMonth:xMonth fromDay:x fromYear:xFromYear arr:fromArr sumkey:@"dayTotal" datekey:@"dayTime"];
                            //下周起始日期
        ...
        //3个月外，按月显示
            ...
            for(int i=_fromMonth;i<=_nowMonth;i++){
                int tmsum = [self getSumOfMonth:i arr:fromArr sumkey:@"dayTotal" datekey:@"dayTime"]<0?0:[self getSumOfMonth:i arr:fromArr sumkey:@"dayTotal" datekey:@"dayTime"];
        ...
        for(int i = _fromYear+1;i<=_nowYear-1;i++){
            for(int j=1;j<=12;j++){
                int tmsum = [self getSumOfMonth:j arr:fromArr sumkey:@"dayTotal" datekey:@"dayTime"]<0?0:[self getSumOfMonth:j arr:fromArr sumkey:@"dayTotal" datekey:@"dayTime"];
                maxY = maxY >= tmsum?maxY:tmsum;
                //x data
