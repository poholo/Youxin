...
@interface WFMonthViewController ()

@end

@implementation WFMonthViewController
...
-(void)downloadComplete:(HttpDownload *)hd{
    ...
    if(dict==NULL||e){
       [SVProgressHUD dismissWithError:@"暂无数据" afterDelay:DELAY_SEC];
        ...
        for(id obj in dict){
            if(obj==NULL||obj==nil||[obj  isKindOfClass:[NSNull class]]){
                NSLog(@"数据有误！");
                return;
            }
        }
        ...
        self.openDate = [[self.useDate description] substringToIndex:10];
        _maxYAll = 0;
        _maxYAdd = 0;
        _maxYCancel = 0;
        //清除数据
