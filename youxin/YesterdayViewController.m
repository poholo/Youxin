...
#import "UIUtils.h"
@interface YesterdayViewController ()<MJRefreshBaseViewDelegate>
{
...
- (void)viewDidLoad
{
    ...
    _tbYesteday = [[UITableView alloc] initWithFrame:CGRectMake(0, 44, 320, self.view.bounds.size.height - 44 - 49)];
    _tbYesteday.delegate = self;
    _tbYesteday.dataSource = self;
    _tbYesteday.backgroundColor = [UIColor clearColor];
    
    _tbYesteday.separatorStyle = UITableViewCellSeparatorStyleNone;
    _tbYesteday.allowsSelection = NO;
    [self.view addSubview:_tbYesteday];
    
    // 下拉刷新
    _header = [[MJRefreshHeaderView alloc] init];
    _header.delegate = self;
    _header.scrollView = _tbYesteday;
    
    // 上拉加载更多
    _footer = [[MJRefreshFooterView alloc] init];
    _footer.delegate = self;
    _footer.scrollView = _tbYesteday;
    ...
    NSString *post = [NSString stringWithFormat:@"iosName=%@&skey=%@&page=%@&parm=%@&timeTag=%@",
...
- (void)refreshViewBeginRefreshing:(MJRefreshBaseView *)refreshView
{
    if(_header == refreshView){
        NSString *post = [NSString stringWithFormat:
                          ...
                          @"done",
                          @"tag2"];
        NSString *hom_url = [NSString stringWithFormat:@"%@%@",HOME_URL,@"Order.php"];
        self.requestType = REQUEST_NO1;
        
        [_mHttpdownload downloadHomeUrl:hom_url postparm:post];
    }
    else{
        NSString *post = [NSString stringWithFormat:
                          @"iosName=%@&skey=%@&page=%@&parm=%@&timeTag=%@",
