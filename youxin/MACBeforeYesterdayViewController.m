//
//  MACBeforeYesterdayViewController.m
//  youxin
//
...
#import "InfoModel.h"
#import "UIUtils.h"
@interface MACBeforeYesterdayViewController ()<MJRefreshBaseViewDelegate>
{
...
@implementation MACBeforeYesterdayViewController
...
- (void)viewDidLoad
{
    ...
    _tbBeforeYesterday= [[UITableView alloc] initWithFrame:CGRectMake(0, 44, self.view.bounds.size.width, self.view.bounds.size.height - 44 -49) style:UITableViewStylePlain];
    _tbBeforeYesterday.delegate = self;
    _tbBeforeYesterday.dataSource = self;
    
    _tbBeforeYesterday.allowsSelection =NO;
    _tbBeforeYesterday.separatorStyle = UITableViewCellSeparatorStyleNone;
    
    
    _tbBeforeYesterday.backgroundColor= [UIColor clearColor];
    [self.view addSubview:_tbBeforeYesterday];
    ...
    _tbBeforeYesterday.separatorColor = UITableViewCellSelectionStyleNone;
    _tbBeforeYesterday.allowsSelection = NO;
    ...
    _header.scrollView = _tbBeforeYesterday;
    
    // 上拉加载更多
    _footer = [[MJRefreshFooterView alloc] init];
    _footer.delegate = self;
    _footer.scrollView = _tbBeforeYesterday;
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(nil== cell){
        cell = [[ManInfoCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"cell" btnno:0 taget:self sel:@selector(cellBtnClikc:) sel2:@selector(tgrclick:)];
...
-(void)reloadTableCellinfo{
    ...
    [_tbBeforeYesterday deleteRowsAtIndexPaths:[NSArray arrayWithObject:self.currentInfoCell.indexpath] withRowAnimation:UITableViewRowAnimationTop];
