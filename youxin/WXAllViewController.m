...
- (void)viewDidLoad
{
    ...
    self.scrollview = [[UIScrollView alloc] initWithFrame:CGRectMake(20+7, _lbYXSum.frame.origin.y + _lbYXSum.frame.size.height+30, 291.0f, self.view.bounds.size.height - 44 -44 -_lbYXSum.frame.origin.y - _lbYXSum.frame.size.height-30)];
...
-(void)manStartTimeAndEndTime{
    NSLog(@"%@",self.usedate);
    _fromYear = [DateUtils getYearFromStr:self.usedate];
    _fromMonth = [DateUtils getMothFromStr:self.usedate];
    _fromDay = [DateUtils getDayFromStr:self.usedate];
        ...
        //3个月内，按周展示
            ...
            switch (splitByMonth) {
                case 1:
                {
                    ...
                    NSDate *openDate = [DateUtils covnertStrToDate:self.usedate];
...
-(void)refreshData{
    ...
    switch (_showType) {
        case SHOW_TYPE_DAY:
        {
            _fdgcYXDis.dataPointsXoffset = 150.0f/4-0.9;
            ...
            break;
        case SHOW_TYPE_WEEK:{
            _fdgcYXDis.dataPointsXoffset = 150.0f/4+1;
            rect = CGRectMake(_fdgcYXDis.frame.origin.x,
                              _scrollview.bounds.size.height-hight,
                              [_xArr count]*30,
                              hight);
    ...
    //时间表示
     _lbTime.text = [NSString stringWithFormat:@"%@ 至 %@",[self.usedate substringToIndex:([self.usedate rangeOfString:@" "]).location],[self convertTimeToString:[NSDate date]]];
