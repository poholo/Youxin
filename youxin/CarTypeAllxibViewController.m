...
#import "CarTypeAllxibViewController.h"
#import "CarCell.h"
...
@interface CarTypeAllxibViewController ()

@end

@implementation CarTypeAllxibViewController
...
//    [src addSubview:_tbCarSort];
    ...
    [NSTimer scheduledTimerWithTimeInterval:1 target:self selector:@selector(startDownload:) userInfo:nil repeats:NO];
    [self modelData];
...
//    _percentageLabel.text = [NSString stringWithFormat:@"%@:%d",[[_carArr objectAtIndex:index] objectForKey:@"keyword"],[[[_carArr objectAtIndex:index] objectForKey:@"vtotal"] intValue]];
   
    NSLog(@"_pieCarAllChart.selectedSliceIndex = %d",_pieCarAllChart.selectedSliceIndex);
}
...
//    _src.contentSize = CGSizeMake(320, 480-44 + [_carArr count]*30);
//    CGRect rect = _tbCarSort.frame;
//    _tbCarSort.frame = CGRectMake(rect.origin.x, rect.origin.y, rect.size.width, 40 + 20* [_carArr count]);
